...
how to create and send patch to linux kernel using git ?

1. git clone git://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git

Install git send-email: This tool is not part of the default Git installation and needs to be installed separately (e.g., sudo apt install git-email on Ubuntu).
Configure git send-email: You need to configure your email settings (SMTP server, user, port, etc.) in your ~/.gitconfig file. For a test run, you can configure it to send to yourself first.
bash

git config --global sendemail.smtpuser myemailaddr@gmail.com
git config --global sendemail.smtpserver smtp.googlemail.com
git config --global sendemail.smtpencryption tls
git config --global sendemail.smtpserverport 587
# Use an application-specific password if using Gmail

Use code with caution.
Configure your email client: Ensure your email client does not wrap lines, mangle whitespace, or send patches as MIME attachments, as kernel maintainers require patches to be sent as inline plain text. 

Step-by-Step Guide

    Make your changes and commit them: Work on a separate branch and create clear, concise commit messages. Each logical change should be a single commit. Use the -s flag to add your Signed-off-by: line, which is required by the Developer's Certificate of Origin.
    bash
...
Generate the patch files: Use git format-patch to convert your commit(s) into patch files. The --cover-letter option is useful for a series of patches.
bash

git format-patch HEAD~N -o outgoing/ --cover-letter
# Replace N with the number of commits you are submitting

Use code with caution.
Check your patches for style violations: The kernel has a strict coding style. Use the scripts/checkpatch.pl script to find potential issues.
bash

./scripts/checkpatch.pl outgoing/*.patch

Use code with caution.
Fix any errors or warnings and amend your commits if necessary (git commit --amend), then regenerate the patches.
Identify the recipients: Use the scripts/get_maintainer.pl script to find the correct maintainers and mailing lists for the code you modified.
bash

./scripts/get_maintainer.pl -f path/to/your/modified/file.c
# Or pass the patch file itself:
./scripts/get_maintainer.pl outgoing/0001-mypatch.patch

Use code with caution.
Send the patches via email: Use git send-email to send the generated patch files. It will automatically use the correct plain text format.
bash

git send-email outgoing/*.patch --to="recipient@example.com" --cc="another@example.com"
# Or automatically populate recipients from the script output:
git send-email --cc-cmd='./scripts/get_maintainer.pl --norolestats' outgoing/*.patch

Use code with caution.
For your first patch, it's a good idea to send it to yourself first to ensure it looks correct and can be applied with git am.
Monitor feedback: Be patient and responsive to review comments. You will likely need to revise your patches and resubmit a new version (e.g., v2, v3) in response to feedback, using the --in-reply-to option to maintain email threading. 

    Submitting Your First Patch to the Linux Kernel and ...
    15 May 2017 — Feedback welcome ( open an issue or email me). * Step 1: Setting up an email client. I mostly use git send-email for se...
    GitHub

Submitting patches: the essential guide to getting your code into the ...
Contents * Development process. * Obtain a current source tree. Describe your changes. Separate your changes. Style-check your cha...
The Linux Kernel Archives
How to configure and use `git send-email` to work with gmail to ...
2 Jul 2021 — See glibc 's contribution instructions here: https://sourceware.org/glibc/wiki/Contribution%20checklist: * Create one or...
Stack Overflow
...
__AI Machine Learning Deep Learning__
...
<jeffrin> hello all
<jeffrin> is image classification AI , ML or DL   is it
<href> yes. all of the above.
<jeffrin> oh
<href> DL is a subset of ML which is a subset of AI, also
<dostoyevsky> jeffrin: people are struggling to find the right words, so they keep changing
<href> and then, i guess the true response is "AI", because "image classification" is a task, not a technique
* techtruth has quit (Ping timeout: 276 seconds)
<jeffrin> i ran a javascript related program and it gave the name of the bird in picture
<jeffrin> with a certain level of confidence
<jeffrin> but when i changed the bird photo .. it keeps scanning
<href> afaik all the current classifications are "DL"
<jeffrin> oh
<jeffrin> but when i changed the bird photo .. it keeps scanning
<href> yes?
* BrindledCat has quit (Quit: Leaving)
<jeffrin> iam using ml5.min.js
<jeffrin> i got the code from  w3schools.com
<href> cool ok
<jeffrin> is ml5.min.js a module which digs into a data to find the name of the bird
<href> iirc it's a friendly wrapper around tensorflow.js. need to implement an architecture and use a model for that.
<href> apparently some birds examples are using MobileNet
<jeffrin> iam using MobileNet
<davros1> Image classification is AI & ML.  It so happens the best solutions utilise DL. As such it the original answer "all of the above" is valid
<davros1> Alexnet in 2012 (?) .. ushered in DL's dominance of the field
<jeffrin> davros1 : oh
<davros1> There are other algos developped to try and do it but DL beats them all
<href> so here ml5 is a wrapper around tensorflow, that implements some things to run clasfficiations models, and you're using the MobileNet model
<href> I think it's a CNN (still DL)
<jeffrin> is MobileNet the name of a nueral network
<davros1> yes
<href> it's a family of models yeah
* techtruth (techtruth@ip68-2-164-46.ph.ph.cox.net) has joined
<href> https://en.wikipedia.org/wiki/MobileNet
<SigmoidFroid> ⇒ MobileNet - Wikipedia | 2025
<davros1> There's all sorts of variations
<davros1> EfficientNet.. ResNeXT blah bah
<jeffrin> ok let me learn things again a come back if i have doubt
<jeffrin> thanks all
* nils` (~nils`@109.164.66.225) has joined
* psydroid2 (~psydroid@user/psydroid) has joined
...
1. __debugging depreciated code__.
2. __debugging natural language grammer__
3. __debugging initialization  file__

   __ special case__
       emacs --debug-init <filename>
       may be a chracter encoding problem  of a directory name

            How to fix the issue
 Rename the directory with the correct characters
The most direct solution is to fix the directory name manually, using a file manager or terminal.

    From the command line:
        Navigate to the ~/JavaScript/ directory using the cd command.
        Use ls to see the actual file path that contains the encoding error. You may see the garbled characters or the correct dash, depending on your terminal's settings.
        Use the mv command to rename the folder. Use tab-completion for the folder name to avoid typing the garbled characters manually. For example:
        sh

        mv "ch05 Javascriptâ\200\224An Objectâ\200\220Based Language 99" "ch05 JavaScript—An Object-Based Language 99"

        Use code with caution.

Using a GUI file manager: Navigate to the ~/JavaScript directory and simply rename the problematic folder. The file manager will handle the encoding correctly and should prevent future errors. 

2. Re-create the file path
If the file path was part of a script or an automated process, you should update the script to use the correct name. Ensure the script itself is saved with UTF-8 encoding. 
3. Update your terminal's character encoding
If this issue is happening frequently, it may be a sign of a larger terminal misconfiguration.

    Check your terminal's character set: Use the locale command to check the settings. A LC_ALL, LC_CTYPE, or LANG variable that is not set to a UTF-8 value (e.g., en_US.UTF-8) is a common cause.
    Set the locale to UTF-8: You can set it for the current session by running export LANG="en_US.UTF-8". For a permanent fix, you may need to add this line to your shell configuration file (~/.bash_profile, ~/.zshrc, etc.). 

4. Clear any cached references (advanced)
In some development environments, a cache might hold the incorrect file path. If you are using a tool that builds or processes your project (e.g., a static site generator or build tool), clearing its cache might resolve the problem. For example, a Next.js project might require deleting the .next folder to force a rebuild with the correct paths. 
AI responses may include mistakes. Learn more
      

4. __debugging module scope__



__git - distributed version control system__


1. To remove the last commit and permanently discard
   all changes made in that commit:
...
3. to force delete a branch
   git branch -D linux-6.17.y

  __ debian development__

  1. skip cleanup process
     dpkg-buildpackage -nc
  2. skip test results
     export DEB_BUILD_OPTIONS='nocheck
  3. skip  key sign   process      related
     dpkg-buildpackage  -b -us   -uc


__YT_DLP__

1. yt-dlp -x --audio-format mp3 --audio-quality 0 --embed-thumbnail --add-metadata -o '%(title)s.%(ext)s' [URL]


__Debian Installer__

1. netinst image to usb stick
   sudo dd bs=4M if=/path/to/debian.iso of=/dev/sdX status=progress oflag=sync
   

__let's encrypt -  SSL - yourdomain.com related__

Install Certbot: sudo apt install certbot python3-certbot-apache.
Run Certbot: sudo certbot --apache -d yourdomain.com -d www.yourdomain.com.
